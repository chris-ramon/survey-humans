...
class AccountManagement::UserController < ApplicationController
  ...
  def dashboard
    ...
    if current_user.profile_id == 2
      ...
      @surveys=Match.where(:match_type_id => 1, :deleted=>0 , :user_id => current_user.id).order('updated_at DESC')
      @exams=Match.where(:match_type_id => 2, :deleted=>0, :user_id => current_user.id).order('updated_at DESC')
    end
  end
end
